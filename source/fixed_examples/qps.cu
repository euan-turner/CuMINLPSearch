...
Problem<double> problem_212()
{
  ...
  x.push_back(problem.var(
      0, 20));  // no explicit upper bound; 20 is implied by c2 since x1,x3 >= 0
  Expr<double> obj = -0.5
      * (x[0] * x[0] + x[1] * x[1] + x[2] * x[2] + x[3] * x[3] + x[4] * x[4]);
  obj = obj - 10.5 * x[0] - 7.5 * x[1] - 3.5 * x[2] - 2.5 * x[3] - 1.5 * x[4]
      - 10 * x[5];
...
Problem<double> problem_219()
{
  ...
  Expr<double> obj = x1 * x2 + x2 * x3 + x3 * x4 + x4 * x5 + x5 * x6 + x6 * x7
      + x7 * x8 + x8 * x9 + x9 * x10 + x1 * x3 + x2 * x4 + x3 * x5 + x4 * x6
      + x5 * x7 + x6 * x8 + x7 * x9 + x8 * x10 + x1 * x9 + x1 * x10 + x2 * x10
      + x1 * x5 + x4 * x7;
  problem.set_objective(-obj);
