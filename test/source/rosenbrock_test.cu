...
// (see make_cycle_context/get_bounds in partition.cuh) with the
// CpuRounding-backed composed ops above instead of the kernel's cu::interval
// device operators. The partitioning scheme itself is fixed; only the
