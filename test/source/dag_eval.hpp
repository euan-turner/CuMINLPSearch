 ...
 */
template<typename T>
auto evaluate(dag::ExprDAG<T> const& graph,
              std::size_t root,
  ...
  for (std::size_t i = 0; i <= root; ++i) {
    ...
    switch (node.op) {
      case Op::Var:
        ...
        break;
      case Op::Const: value[i] = node.payload.constant; break;
      case Op::Add:   value[i] = arg(0) + arg(1); break;
      case Op::Sub:   value[i] = arg(0) - arg(1); break;
      case Op::Mul:   value[i] = arg(0) * arg(1); break;
      case Op::Div:   value[i] = arg(0) / arg(1); break;
      case Op::Min:   value[i] = std::fmin(arg(0), arg(1)); break;
      case Op::Max:   value[i] = std::fmax(arg(0), arg(1)); break;
      case Op::Neg:   value[i] = -arg(0); break;
      case Op::Sqr:   value[i] = arg(0) * arg(0); break;
      case Op::Exp:   value[i] = std::exp(arg(0)); break;
      case Op::Log:   value[i] = std::log(arg(0)); break;
      case Op::Sqrt:  value[i] = std::sqrt(arg(0)); break;
      case Op::Sin:   value[i] = std::sin(arg(0)); break;
...
template<typename T>
auto evaluate_objective(dag::Problem<T> const& problem, std::vector<T> const& point)
    -> T
{
