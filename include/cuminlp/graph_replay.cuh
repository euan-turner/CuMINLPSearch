...
__global__ void partition_variables_kernel(
  ...
  for (std::size_t r = tid; r < n_regions; r += num_threads) {
    auto ctx = partition::make_slot_context<CycleSize>(r, slot_var_ids, slot_fan_out, slot_kind);
...
std::uint64_t sample_hash(std::size_t vid, std::size_t i, std::size_t region, std::size_t salt)
{
...
template<typename T>
__device__ __forceinline__
T sample_discrete_from_interval(T lb, T ub, std::size_t vid, std::size_t i,
                                std::size_t region, std::size_t salt)
{
  std::uint64_t x = sample_hash(vid, i, region, salt);
  ...
  T lo = ceil(lb);
  T hi = floor(ub);
  if (hi < lo) return lo;  // sub-box narrower than 1 unit; no integer strictly inside, clamp

  std::size_t count = static_cast<std::size_t>(hi - lo) + 1;
  std::size_t idx = static_cast<std::size_t>(x % static_cast<std::uint64_t>(count));
  ...
  for (std::size_t r = tid; r < n_regions; r += num_threads) {
    ...
    for (std::size_t vid = 0; vid < n_vars; ++vid) {
      ...
      for (std::size_t i = 0; i < SamplePoints; ++i) {
        T p = discrete ? sample_discrete_from_interval(v.lb, v.ub, vid, i, r, salt)
...
struct MinOp {
  ...
  template<typename T> static __device__ T apply(T a, T b) { return cu::intrinsic::min<T>(a, b); }
};
struct MaxOp {
  ...
  template<typename T> static __device__ T apply(T a, T b) { return cu::intrinsic::max<T>(a, b); }
};
struct NegOp {
  template<typename T> static __device__ cu::interval<T> apply(cu::interval<T> a) { return -a; }
  template<typename T> static __device__ T apply(T a) { return -a; }
};
struct SqrOp {
  template<typename T> static __device__ cu::interval<T> apply(cu::interval<T> a) { return sqr(a); }
  template<typename T> static __device__ T apply(T a) { return a * a; }
};
struct ExpOp {
  template<typename T> static __device__ cu::interval<T> apply(cu::interval<T> a) { return exp(a); }
  template<typename T> static __device__ T apply(T a) { return std::exp(a); }
};
struct LogOp {
  template<typename T> static __device__ cu::interval<T> apply(cu::interval<T> a) { return log(a); }
  template<typename T> static __device__ T apply(T a) { return std::log(a); }
};
struct SqrtOp {
  template<typename T> static __device__ cu::interval<T> apply(cu::interval<T> a) { return sqrt(a); }
  template<typename T> static __device__ T apply(T a) { return std::sqrt(a); }
};
struct SinOp {
  template<typename T> static __device__ cu::interval<T> apply(cu::interval<T> a) { return sin(a); }
  template<typename T> static __device__ T apply(T a) { return std::sin(a); }
};
struct CosOp {
  template<typename T> static __device__ cu::interval<T> apply(cu::interval<T> a) { return cos(a); }
  template<typename T> static __device__ T apply(T a) { return std::cos(a); }
};
struct TanhOp {
  template<typename T> static __device__ cu::interval<T> apply(cu::interval<T> a) { return tanh(a); }
  template<typename T> static __device__ T apply(T a) { return std::tanh(a); }
};
struct AbsOp {
  template<typename T> static __device__ cu::interval<T> apply(cu::interval<T> a) { return abs(a); }
  template<typename T> static __device__ T apply(T a) { return std::fabs(a); }
};
...
template<class OpTag> struct op_code;
template<> struct op_code<AddOp>  { static constexpr Op value = Op::Add; };
template<> struct op_code<SubOp>  { static constexpr Op value = Op::Sub; };
template<> struct op_code<MulOp>  { static constexpr Op value = Op::Mul; };
template<> struct op_code<DivOp>  { static constexpr Op value = Op::Div; };
template<> struct op_code<MinOp>  { static constexpr Op value = Op::Min; };
template<> struct op_code<MaxOp>  { static constexpr Op value = Op::Max; };
template<> struct op_code<NegOp>  { static constexpr Op value = Op::Neg; };
template<> struct op_code<SqrOp>  { static constexpr Op value = Op::Sqr; };
template<> struct op_code<ExpOp>  { static constexpr Op value = Op::Exp; };
template<> struct op_code<LogOp>  { static constexpr Op value = Op::Log; };
template<> struct op_code<SqrtOp> { static constexpr Op value = Op::Sqrt; };
template<> struct op_code<SinOp>  { static constexpr Op value = Op::Sin; };
template<> struct op_code<CosOp>  { static constexpr Op value = Op::Cos; };
template<> struct op_code<TanhOp> { static constexpr Op value = Op::Tanh; };
template<> struct op_code<AbsOp>  { static constexpr Op value = Op::Abs; };
...
template<typename T>
__device__ cu::interval<T> ipow_value(cu::interval<T> a, int exponent) { return pown(a, exponent); }
template<typename T>
__device__ T ipow_value(T a, int exponent) { return ipow_scalar(a, exponent); }
...
__device__ bool is_feasible(cu::interval<T> v, Cmp cmp, T rhs) {
  if (cmp == Cmp::LE) return v.lb <= rhs;
  if (cmp == Cmp::EQ) return v.lb <= rhs && rhs <= v.ub;
  return false;
}
template<typename T>
__device__ bool is_feasible(T v, Cmp cmp, T rhs) {
  if (cmp == Cmp::LE) return v <= rhs;
  if (cmp == Cmp::EQ) return almost_equal(v, rhs);
  return false;
}

// Writes the shared, absorbing-zero feasible[] flag; concurrent writes agree,
// so no atomics needed. This is strictly an infeasibility check, feasible[i] stays 1
// if at least part of the bound on the lhs is feasible.
template<typename T, typename V>
...
// the same node shape as the interval graph despite there being nothing to bound.
template<typename T>
__device__ T lower_bound_of(cu::interval<T> v) { return v.lb; }
template<typename T>
__device__ T lower_bound_of(T v) { return v; }
template<typename T>
__device__ T upper_bound_of(cu::interval<T> v) { return v.ub; }
template<typename T>
__device__ T upper_bound_of(T v) { return v; }
...
// Must run after every constraint's feasibility_check_kernel has written feasible[] 
template<typename T>
__global__ void mask_infeasible_kernel(const T* __restrict__ obj_ub,
...
// V is the value type of a node's buffer, either cu::interval<T> for the interval graph,
// or T for the sample/exact graphs. Below the root node partitioning/sampling/enumerating
// node, the graphs are identical as the kernels and operators are overloaded.
// Exact (only meaningful when V=T) selects the deterministic enumerate_points_kernel
// instead of sample_points_kernel's random sampling -- see ExactGraphReplay.
template<typename T, typename V, std::size_t CycleSize, std::size_t PartitionNum,
         std::size_t SamplePoints = 1, bool Exact = false>
class GraphBuilder {
  static_assert(std::is_same_v<V, T> || std::is_same_v<V, cu::interval<T>>,
...
public:
   ...
   * @param slot_var_ids Device buffer, CycleSize entries: which variable each slot acts on
   * @param slot_fan_out Device buffer, CycleSize entries: fan-out of each slot
   * @param slot_kind Device buffer, CycleSize entries: operation each slot performs
  ...
  GraphBuilder(const Problem<T>& problem, cu::interval<T>* domain_buffer, std::size_t n_regions,
              const std::size_t* slot_var_ids, const int* slot_fan_out, const SlotKind* slot_kind,
              const VarKind* var_kinds)
      : problem_(problem)
      , n_regions_(n_regions)
      , n_elems_(is_point ? n_regions * SamplePoints : n_regions) // the number of V-typed slots to operate over
      , block_(256)
      ...
      , root_grid_(static_cast<unsigned int>(detail::ceil_div(n_regions_, 256)))
  {
    ...
    detail::check(cudaMemcpy(var_buffers_device_, var_buffer_list.data(),
                             n_vars * sizeof(V*), cudaMemcpyHostToDevice),
                 "cudaMemcpy");

    if constexpr (is_point && Exact) {
      root_node_ = detail::add_kernel_node(
          graph_, {}, enumerate_points_kernel<T, CycleSize>, root_grid_, block_,
          domain_buffer, slot_var_ids, slot_fan_out, slot_kind, var_buffers_device_, n_vars,
          n_regions_);
    } else if constexpr (is_point) {
      root_node_ = detail::add_kernel_node(
          graph_, {}, sample_points_kernel<T, CycleSize, SamplePoints>, root_grid_,
          block_, domain_buffer, slot_var_ids, slot_fan_out, slot_kind, var_kinds,
          var_buffers_device_, n_vars, n_regions_, std::size_t {0});
    } else {
      root_node_ = detail::add_kernel_node(
          graph_, {}, partition_variables_kernel<T, CycleSize>, root_grid_, block_,
          domain_buffer, slot_var_ids, slot_fan_out, slot_kind, var_buffers_device_, n_vars,
          n_regions_);
    }

    for (const auto& node : problem_.graph.nodes) {
      if (node.op == Op::Var) producer_nodes_[node.id] = root_node_;
    }
  }
...
private:
  ...
  cudaGraphNode_t wire_binary(std::size_t id) {
    ...
    if (node.in.size() != 2) {
      throw std::runtime_error("wire_binary called on a node without exactly two operands");
    }
    ...
    if (!lhs_const && !rhs_const) {
      ...
      return detail::add_kernel_node(graph_, deps, binary_op_kernel<V, T, BinaryOp>,
                                     grid_, block_, buffers_[lhs_id], buffers_[rhs_id],
                                     buffers_[id], n_elems_);
    }
    if (!lhs_const && rhs_const) {
      cudaGraphNode_t lhs_node = ensure_node(lhs_id);
      T rhs_val = problem_.graph.nodes[rhs_id].payload.constant;
      return detail::add_kernel_node(graph_, {lhs_node}, binary_op_scalar_rhs_kernel<V, T, BinaryOp>,
    ...
    if (lhs_const && !rhs_const) {
      ...
      cudaGraphNode_t rhs_node = ensure_node(rhs_id);
      return detail::add_kernel_node(graph_, {rhs_node}, binary_op_scalar_lhs_kernel<V, T, BinaryOp>,
  ...
  cudaGraphNode_t wire_unary(std::size_t id) {
    ...
    if (node.in.size() != 1) {
      throw std::runtime_error("wire_unary called on a node without exactly one operand");
    }
    ...
    buffers_[id] = detail::alloc_device<V>(n_elems_);
    return detail::add_kernel_node(graph_, {operand_node}, unary_op_kernel<V, T, UnaryOp>, grid_,
                                   block_, buffers_[operand_id], buffers_[id], n_elems_);
  }

  cudaGraphNode_t wire_ipow(std::size_t id) {
    const DAGNode<T>& node = problem_.graph.nodes[id];
    if (node.op != Op::IPow) {
      throw std::runtime_error("wire_ipow called on a node whose op is not Op::IPow");
    }
    if (node.in.size() != 1) {
      throw std::runtime_error("wire_ipow called on a node without exactly one operand");
    }
    std::size_t operand_id = node.in[0];
    if (problem_.graph.nodes[operand_id].op == Op::Const) {
    ...
    cudaGraphNode_t operand_node = ensure_node(operand_id);
    buffers_[id] = detail::alloc_device<V>(n_elems_);
    return detail::add_kernel_node(graph_, {operand_node}, ipow_kernel<T, V>,
                                   grid_, block_, buffers_[operand_id], node.payload.int_exp,
                                   buffers_[id], n_elems_);
  }
...
// Owns the entire graph replay for a problem and value-type V (either T or cu::interval<T>)
// This includes:
...
// Exact (only meaningful when V=T) selects the deterministic enumerate_points_kernel
// as the root node instead of sample_points_kernel's random sampling -- see
// ExactGraphReplay/GraphBuilder.
template<typename T, typename V, std::size_t CycleSize, std::size_t PartitionNum,
         std::size_t SamplePoints = 1, std::size_t EnumerateCap = PartitionNum,
         bool Exact = false>
class GraphReplay {
  static constexpr bool is_point = std::is_same_v<V, T>;
...
public:
  ...
  // between launches.
  static GraphReplay build(const Problem<T>& problem, const Composition<CycleSize>& composition) {
    GraphReplay replay;
    replay.composition_ = composition;
    replay.n_regions_ = composition_fan_out<PartitionNum, EnumerateCap>(composition);
    ...
    for (std::size_t j = 0; j < CycleSize; ++j) {
      fan_out_host[j] = static_cast<int>(slot_fan_out<PartitionNum, EnumerateCap>(composition[j]));
    }
    detail::check(cudaMemcpy(replay.slot_fan_out_device_, fan_out_host.data(), CycleSize * sizeof(int),
                             cudaMemcpyHostToDevice),
                 "cudaMemcpy");
    detail::check(cudaMemcpy(replay.slot_kind_device_, composition.data(), CycleSize * sizeof(SlotKind),
                             cudaMemcpyHostToDevice),
                 "cudaMemcpy");
    detail::check(cudaMemcpy(replay.var_kinds_device_, problem.var_kinds.data(),
                             replay.n_vars_ * sizeof(VarKind), cudaMemcpyHostToDevice),
                 "cudaMemcpy");

    GraphBuilder<T, V, CycleSize, PartitionNum, SamplePoints, Exact> builder(
        problem, replay.domain_buffer_, replay.n_regions_, replay.slot_var_ids_device_,
        replay.slot_fan_out_device_, replay.slot_kind_device_, replay.var_kinds_device_);
    ...
    replay.feasible_buffer_ = detail::alloc_device<unsigned char>(replay.n_elems_);
    ...
    for (const auto& constraint : problem.constraints) {
      ...
      cudaGraphNode_t feasibility_node = detail::add_kernel_node(
          builder.graph(), {constraint_producer, feasible_memset_node},
          feasibility_check_kernel<T, V>, builder.grid(), builder.block(),
          builder.buffer_for(constraint.root_id), constraint.cmp, constraint.rhs,
          replay.feasible_buffer_, replay.n_elems_);
      feasibility_nodes.push_back(feasibility_node);
    }

    detail::add_kernel_node(builder.graph(), {obj_producer},
                            objective_extract_kernel<T, V>, builder.grid(),
                            builder.block(), builder.buffer_for(problem.objective_root),
                            replay.obj_lb_buffer_, replay.n_elems_);

    cudaGraphNode_t obj_ub_node = detail::add_kernel_node(
        builder.graph(), {obj_producer}, objective_extract_ub_kernel<T, V>,
        builder.grid(), builder.block(), builder.buffer_for(problem.objective_root),
        replay.obj_ub_buffer_, replay.n_elems_);
    ...
    cudaGraphNode_t mask_node = detail::add_kernel_node(
        builder.graph(), mask_deps, mask_infeasible_kernel<T>, builder.grid(), builder.block(),
        replay.obj_ub_buffer_, replay.feasible_buffer_, replay.masked_ub_buffer_, replay.n_elems_);
    ...
    detail::check(cudaGraphAddChildGraphNode(&reduce_node, builder.graph(), &mask_node, 1,
                                             captured_graph),
  ...
  void set_domain(std::span<const cu::interval<T>> domain,
    ...
    detail::check(cudaMemcpy(domain_buffer_, domain.data(), n_vars_ * sizeof(cu::interval<T>),
                             cudaMemcpyHostToDevice),
                 "cudaMemcpy");
    detail::check(cudaMemcpy(slot_var_ids_device_, var_ids.data(), CycleSize * sizeof(std::size_t),
                             cudaMemcpyHostToDevice),
                 "cudaMemcpy");
    ...
    if constexpr (is_point && Exact) {
      void* kernel_args[] = {
          &domain_buffer_,    &slot_var_ids_device_, &slot_fan_out_device_,
          &slot_kind_device_, &var_buffers_device_,  &n_vars_,
          &n_regions_,
      };
      params.func = reinterpret_cast<void*>(enumerate_points_kernel<T, CycleSize>);
      ...
      void* kernel_args[] = {
          &domain_buffer_,      &slot_var_ids_device_, &slot_fan_out_device_,
          &slot_kind_device_,   &var_kinds_device_,    &var_buffers_device_,
          &n_vars_,             &n_regions_,           &salt,
      };
      params.func = reinterpret_cast<void*>(sample_points_kernel<T, CycleSize, SamplePoints>);
      params.kernelParams = kernel_args;
      detail::check(cudaGraphExecKernelNodeSetParams(exec_, root_node_, &params),
                   "cudaGraphExecKernelNodeSetParams");
    } else {
      ...
      params.func = reinterpret_cast<void*>(partition_variables_kernel<T, CycleSize>);
      params.kernelParams = kernel_args;
      detail::check(cudaGraphExecKernelNodeSetParams(exec_, root_node_, &params),
                   "cudaGraphExecKernelNodeSetParams");
    }
  ...
  void launch(cudaStream_t stream) {
    ...
    detail::check(cudaMemcpy(feasible_host_.data(), feasible_buffer_,
                             n_elems_ * sizeof(unsigned char), cudaMemcpyDeviceToHost),
                 "cudaMemcpy");
...
private:
  ...
  void free_resources() {
    ...
    if (slot_var_ids_device_) cudaFree(slot_var_ids_device_);
    if (slot_fan_out_device_) cudaFree(slot_fan_out_device_);
    if (slot_kind_device_) cudaFree(slot_kind_device_);
    if (var_kinds_device_) cudaFree(var_kinds_device_);
  ...
  V** var_buffers_device_ = nullptr;
  std::size_t* slot_var_ids_device_ = nullptr; // device, CycleSize entries, written by set_domain()
  int* slot_fan_out_device_ = nullptr;         // device, CycleSize entries, fixed by composition_
  SlotKind* slot_kind_device_ = nullptr;       // device, CycleSize entries, fixed by composition_
  VarKind* var_kinds_device_ = nullptr;        // device, n_vars_ entries, fixed by the problem
...
using IntervalGraphReplay = GraphReplay<T, cu::interval<T>, CycleSize, PartitionNum, 1, EnumerateCap>;

template<typename T, std::size_t CycleSize, std::size_t PartitionNum, std::size_t SamplePoints,
        std::size_t EnumerateCap = PartitionNum>
using PointGraphReplay = GraphReplay<T, T, CycleSize, PartitionNum, SamplePoints, EnumerateCap>;
...
template<typename T, std::size_t CycleSize, std::size_t PartitionNum,
        std::size_t EnumerateCap = PartitionNum>
using ExactGraphReplay = GraphReplay<T, T, CycleSize, PartitionNum, 1, EnumerateCap, /*Exact=*/true>;
