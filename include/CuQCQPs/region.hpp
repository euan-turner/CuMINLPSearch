...
// RegionHistory is append only, when a region is selected, the full 2*n interval is added
// may be worth converting to a min-heap in the future
...
struct Region {
  ...
  uint32_t iter_idx;    // index into region list, corresponds to region's direct parent
...
template <std::size_t NDIMS>
void materialise_region(
    const Region& region,
    ExplicitRegion<NDIMS>& out)
{
  if (region.iter_idx == 0) {
    // first region
    std::fill(std::begin(out.lower), std::end(out.lower), std::numeric_limits<double>::lowest());
    std::fill(std::begin(out.upper), std::end(out.upper), std::numeric_limits<double>::max());
  } else {
    // TODO
